---
title: "Statistiques et data mining - Master Hydrosystèmes et Bassins Versants"
...
-   La description de jeux de données, c'est à dire le synthétiser ou d'explorer son potentiel et ses limites. Il s'agit de mettre en évidence les informations que peuvent contenir le jeu de données.

-   La prédiction de nouvelles informations, soit l'extrapolation de nouvelles données à partir des associations ou relations mises en évidence dans les données. Ces informations peuvent être qualitatives (classement ou scoring) ou quantitative (prédiction).

## Objectifs du cours

Acquérir des outils de description et d'analyse de données et une démarche critique sur les données et les résultats obtenus.
...
Un outil très complet gratuit et open source. Permet de reproduire facilement les analyses et assure une reproductibilité des résultats.
:::
...
Approche data mining et ses application

## Données utilisées

Données hydromorphologiques issus du protocole CARHYCE. [IED CARHYCE](https://analytics.huma-num.fr/ied_carhyce/)

## Importons les pakages et le jeu de données

\

```{r, echo=TRUE, collapse=FALSE, results='hide'}
# install all necessary packages
# install.packages(c("ggplot2", "dplyr", "tidyr", "tibble", "janitor", "skimr", "dataMaid", "plotly", "e1071", "gmodels", "ggpmisc", "nortest", "FactoMineR", "factoextra", "gtsummary", "explor"))
```
...
carhyce <- carhyce_brute %>% # %>% = pipe operator from dplyr
...
```{r, echo=TRUE, collapse=FALSE}
# Histogram
hist(carhyce_stat$surface_bv_km2, main = "Surfaces de bassins versants", xlab = "Surface de bassins versants (km2)", col = "lightblue")
```
:::
...
```{r, echo=TRUE, collapse=FALSE}
# Boxplot
boxplot(carhyce_stat$surface_bv_km2, main = "Surfaces de bassins versants", ylab = "Surface de bassins versants (km2)", col = "lightblue")
...
- Peut parfois sous-estimer le nombre de classes nécessaires, surtout pour des échantillons de taille réduite ou pour des distributions non normales.
...
```{r, echo=TRUE, collapse=FALSE}
# print decile values
print(quantile(carhyce_stat$surface_bv_km2, probs = seq(0, 1, 0.1), na.rm = TRUE))
```
...
```{r, echo=TRUE, collapse=FALSE}
# keep the data below the 90th percentile
...
h <- hist(carhyce_stat$surface_bv_km2, col = "lightblue", main = "Surface des bassins versants", xlab = "Surface de bassins versants (km2)") 
...
```{r, echo=TRUE, collapse=FALSE}
# Boxplot
boxplot(carhyce_stat$surface_bv_km2, main = "Surfaces de bassins versants", ylab = "Surface de bassins versants (km2)", col = "lightblue", range = 0)
points(1,mean(carhyce_stat$surface_bv_km2,na.rm=T),pch=16,col="red")
```
:::
:::
...
-  Distribution symétrique centrée sur la moyenne et dont la forme est déterminée par l'écart-type.
...
plot(x, hx, type="l", lwd=2, col="black", xlab="x", ylab="Density", main="Density of normal distribution", ylim=c(0,0.8), xlim=c(-5,5))
...
## Distribution normale? - QQ-plot


```{r, echo=TRUE, collapse=FALSE, fig.height=4}
# QQ-plot
qqnorm(carhyce_stat$surface_bv_km2, 
       main = "Normal Q-Q Plot - Surface des bassins versants")
qqline(carhyce_stat$surface_bv_km2)
```
...
- p-value $<$ 0.05 => la distribution ne suit pas une loi normale.
...
[Fréquences partielles sur effectif total]{style="font-size: 0.7em;"}
...
[Discrétiser la variable quantitative.]{style="font-size: 0.7em;"}

```{r, echo=TRUE, collapse=FALSE}
bins <- nclass.Sturges(carhyce_stat$largeur_mouillee_qb_m) # set breaks with Sturges
carhyce_stat$largeur_mouillee_qb_m_sturges <- cut(carhyce_stat$largeur_mouillee_qb_m, breaks = bins, include.lowest = TRUE, right = FALSE) # new column with breaks
tab <- table(carhyce_stat$continuite_ripisylve_rive_gauche, carhyce_stat$largeur_mouillee_qb_m_sturges) # contingency table
