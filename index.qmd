...
Le $\chi^2$ représente la différence des deux tables que l'on compare au $\chi^2$ critique pour déterminer si on peut rejeter $H_0$ pour un risque $\alpha$ donné. Si $\chi^2$ > $\chi^2 critique$, on rejette $H_0$. Voir [table de $\chi^2$ par Fabrice Larribe](http://fabricelarribe.uqam.ca/resources/Tables/Table-Chi2-Print.pdf){target="_blank"}

## Chi2 - interprétation

```{r, echo=TRUE, collapse=FALSE}
chisq.test(table(carhyce_stat$continuite_ripisylve_rive_gauche, carhyce_stat$continuite_ripisylve_rive_droite))
```
- La p-value représente la probabilité d'obtenir un résultat aussi extrême que celui observé, dans les conditions de $H_0$.

- Cette situation est très improbable. On peut rejeter $H_0$ et accepter $H_1$ avec une probabilité de se tromper de 5%.
...
- Coefficients du modèle avec erreur standard et test de significativité  :
  ...
  - Intersept : l'ordonnée à l'origine
  - Variable : la pente
...
<details>
  <summary>Interprétation</summary>
La médiane des résidus est plutôt proche de 0, l'interval inter-quartile et l'erreur standard sont d'environ 5m. La majeure partie des valeurs observées sont donc proches du modèle. Les valeur minimum et maximum montrent cependant de très forts écarts.
Le modèle  prend la forme de l'équation $largeur = 7.54+0.39*Débit$ Les coefficients ont de faibles dispersions et une forte significativité, de même que le test de Fisher. Il existe bien une relation linéaire de la largeur en fonction de du débit.
...
La distribution des résidus présente une sureprésentation des valeurs fortes par rapport à la normale, soit une dissymétrie à droite.
</details>
...
- Hypothèse alternative $H_a$ : la pente est différente de 0 (relation linéaire entre les deux variables)
- Test de Student :
